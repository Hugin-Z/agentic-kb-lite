...
- **不替使用者预调 recipe 参数**(v0.4 recipe 是能力齐全的 baseline 骨架 + experimental 定位:真实语料的检索增益验证 / 调参由实际使用者用自有语料完成,本仓库只提供确定性变换正确性的合成单测;不做语义级清洗 / 字段抽取 / 表头语义识别 / 跨 3+ 表合并 —— 真要精细化是未来另写独立 recipe)
